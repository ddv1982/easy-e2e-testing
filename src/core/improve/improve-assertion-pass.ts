...
import { augmentCandidatesWithSnapshotInventory } from "./improve-assertion-inventory.js";
import {
...
export async function runImproveAssertionPass(input: {
  ...
  const inventory = augmentCandidatesWithSnapshotInventory({
  ...
  rawAssertionCandidates = inventory.candidates;
  const inventoryStepsEvaluated = inventory.inventoryStepsEvaluated;
  const inventoryCandidatesAdded = inventory.inventoryCandidatesAdded;
  const inventoryGapStepsFilled = inventory.inventoryGapStepsFilled;
  ...
  if (input.applyAssertions && input.page) {
    ...
    for (const outcome of allOutcomes) {
      ...
      if (outcome.applyMessage !== undefined) {
        outcomeEntry.applyMessage = outcome.applyMessage;
      }
      outcomeByCandidate.set(outcome.candidateIndex, outcomeEntry);
